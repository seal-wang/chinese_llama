...
## 🥇总结
经过训练和微调，模型最终效果只能说很勉强，这固然有参数量小的原因，但是也有一些可以进一步优化的点，比如进一步精简Embedding层的参数，增加Attention和FNN层的参数；在微调后继续进行DPO来优化模型的表达；增加Dropout操作，减小参数的冗余；进一步筛选SFT语料，去除模型能力之外的一些Instruction,如翻译、编程等。
